...
def upgrade() -> None:
    ...
    # Add missing columns to materials table (these were missing from the original migration)
    conn = op.get_bind()

    # Check if main_topic column exists, if not add it
    result = conn.execute(sa.text("SELECT column_name FROM information_schema.columns WHERE table_name='materials' AND column_name='main_topic'"))
    if not result.fetchone():
        op.add_column('materials', sa.Column('main_topic', sa.String(100), nullable=True))
        op.add_column('materials', sa.Column('main_topic_zh', sa.String(100), nullable=True))
        op.add_column('materials', sa.Column('topic_zh', sa.String(100), nullable=True))
        op.add_column('materials', sa.Column('prompt_zh', sa.Text(), nullable=True))
        op.add_column('materials', sa.Column('prompt_en', sa.Text(), nullable=True))
        op.add_column('materials', sa.Column('effect_prompt', sa.Text(), nullable=True))
        op.add_column('materials', sa.Column('effect_prompt_zh', sa.Text(), nullable=True))
        op.add_column('materials', sa.Column('input_video_url', sa.String(500), nullable=True))
        # Note: lookup_hash is added by a later migration (bae2b07d4b94)
...
def downgrade() -> None:
    # ### commands auto generated by Alembic - please adjust! ###
    # Drop indexes we may have created
    conn = op.get_bind()
    result = conn.execute(sa.text("SELECT column_name FROM information_schema.columns WHERE table_name='materials' AND column_name='main_topic'"))
    if result.fetchone():
        ...
        # Drop added columns (lookup_hash handled by bae2b07d4b94)
        op.drop_column('materials', 'input_video_url')
        op.drop_column('materials', 'effect_prompt_zh')
        op.drop_column('materials', 'effect_prompt')
        op.drop_column('materials', 'prompt_en')
        op.drop_column('materials', 'prompt_zh')
        op.drop_column('materials', 'topic_zh')
        op.drop_column('materials', 'main_topic_zh')
        op.drop_column('materials', 'main_topic')
