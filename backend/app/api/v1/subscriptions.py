...
#
# 2026-07 policy: allowed_models is the SAME full catalog on every paid plan
# — any subscriber with enough credits may use any model (per-model credit
# pricing covers the upstream cost). Plans differentiate on monthly credits,
# max_concurrent_generations, and priority_queue (which the load_governor
# now actually enforces under heavy load). NOTE: ensure_vidgo_plans is
# insert-only, so pre-existing DB rows keep their old allowed_models — the
