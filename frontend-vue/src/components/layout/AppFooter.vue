...
<template>
  <footer style="background: #0a0a0f; border-top: 1px solid rgba(255,255,255,0.06);">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
      <div class="grid grid-cols-2 md:grid-cols-6 gap-10">
        <!-- Brand -->
        <div class="col-span-2">
          <RouterLink to="/" class="flex items-center gap-2.5 mb-4">
          ...
          <p class="text-sm leading-relaxed max-w-xs mb-6" style="color: #6b6b8a;">
            {{ t('footer.tagline') }}
          </p>
          <div class="flex items-center gap-3">
            <a href="#" class="w-9 h-9 rounded-full flex items-center justify-center transition-colors" style="background: rgba(255,255,255,0.06); color: #6b6b8a;" @mouseenter="($event.target as HTMLElement).style.background='rgba(22,119,255,0.12)'; ($event.target as HTMLElement).style.color='#1677ff'" @mouseleave="($event.target as HTMLElement).style.background='rgba(255,255,255,0.06)'; ($event.target as HTMLElement).style.color='#6b6b8a'">
        ...
        <div>
          ...
          <ul class="space-y-2.5">
            ...
            <li><RouterLink to="/tools/upscale" class="text-sm transition-colors" style="color: #6b6b8a;" @mouseenter="($event.target as HTMLElement).style.color='#1677ff'" @mouseleave="($event.target as HTMLElement).style.color='#6b6b8a'">{{ t('lp.allTools.hdUpscale.name') }}</RouterLink></li>
        ...
        <!-- Company -->
        <div>
          <h4 class="font-semibold mb-4 text-sm" style="color: #e8e8f0;">{{ t('footer.company') }}</h4>
          <ul class="space-y-2.5">
            <li><a href="#" class="text-sm transition-colors" style="color: #6b6b8a;">{{ t('footer.about') }}</a></li>
            <li><RouterLink to="/pricing" class="text-sm transition-colors" style="color: #6b6b8a;">{{ t('footer.pricing') }}</RouterLink></li>
            <li><a href="#" class="text-sm transition-colors" style="color: #6b6b8a;">{{ t('footer.contact') }}</a></li>
            <li><a href="#" class="text-sm transition-colors" style="color: #6b6b8a;">{{ t('footer.blog') }}</a></li>
            <li><a href="#" class="text-sm transition-colors" style="color: #6b6b8a;">{{ t('footer.affiliate') }}</a></li>
          </ul>
        </div>

        <!-- Legal -->
        <div>
          <h4 class="font-semibold mb-4 text-sm" style="color: #e8e8f0;">{{ t('footer.legal') }}</h4>
          <ul class="space-y-2.5">
            <li><a href="#" class="text-sm transition-colors" style="color: #6b6b8a;">{{ t('footer.terms') }}</a></li>
            <li><a href="#" class="text-sm transition-colors" style="color: #6b6b8a;">{{ t('footer.privacy') }}</a></li>
            <li><a href="#" class="text-sm transition-colors" style="color: #6b6b8a;">{{ t('footer.cookies') }}</a></li>
            <li><a href="#" class="text-sm transition-colors" style="color: #6b6b8a;">{{ t('footer.refunds') }}</a></li>
          </ul>
        </div>
      </div>
      ...
      <div class="mt-12 pt-8 flex flex-col md:flex-row items-center justify-between gap-4" style="border-top: 1px solid rgba(255,255,255,0.06);">
        <p class="text-sm" style="color: #6b6b8a;">{{ '\u00A9' }} 2025 VidGo AI. All rights reserved.</p>
        <div class="flex items-center gap-3">
          <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold" style="background: rgba(22,119,255,0.1); color: #69b1ff; border: 1px solid rgba(22,119,255,0.2);">
