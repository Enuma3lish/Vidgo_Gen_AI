...
// NOTE (2026-06-15): buildAlternateLocales() was removed. The site now uses a
// single canonical URL per page (the param-less path) instead of advertising
// ?lang= locale variants as separate indexable URLs. Emitting per-locale
// hreflang while the canonical pointed at the param-less path made Google mark
// every ?lang= URL as "Alternate page with proper canonical tag". applySeo()
// still calls removeAllHreflang() each navigation, so any previously-injected
// hreflang is cleared. If true per-language indexing is ever wanted, switch to
// path-prefixed locale URLs (/en/, /ja/) with self-referential canonicals.
