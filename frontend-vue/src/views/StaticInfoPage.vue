...
const pages = computed<Record<string, { title: string; body: string[] }>>(() => ({
  ...
  terms: {
    ...
    body: isZh.value
      ...
      : [
          ...
          '4. Subscriptions and Billing. Paid plans are described in the dashboard. Payments are processed by third-party payment service providers including PayPal and ECPay, which issue receipts and invoices. Subscriptions auto-renew each billing period and may be cancelled anytime in account settings to prevent the next charge.',
  ...
  privacy: {
    ...
    body: isZh.value
      ...
      : [
          ...
          '2. Data We Collect. (a) Account data: email, hashed password, language/theme preferences, sign-in history, device fingerprint; (b) Payment data: transaction IDs, subscription plan, invoice title and tax ID (card data is handled exclusively by PayPal / ECPay; VidGo does not see full card numbers); (c) Usage data: uploaded media, generation prompts, generated outputs, credit transactions; (d) Support data: content of correspondence with our team.',
          ...
          '5. Sub-processors. We rely on: payment (PayPal, ECPay), cloud infrastructure (Google Cloud Run, Cloud Storage, Cloud SQL), AI providers (PiAPI, Pollo, A2E.ai, Google Vertex AI, Gemini), invoicing (Giveme), email (SMTP). Each receives only the minimum data needed to perform its function.',
  ...
  refunds: {
    ...
    body: isZh.value
      ? [
          `最後更新：${LAST_UPDATED}`,
          '1. 適用範圍。本退款政策適用於所有透過 VidGo 平台購買之訂閱方案與點數加購。',
          '2. 訂閱退款窗口。新訂閱用戶於首次扣款後 14 個自然日內，且尚未消費當期訂閱點數超過 30%，可向客服申請全額退款。超過 14 天或已消費超過 30% 訂閱點數者，視為已使用該服務，不予退款。',
          '3. 自動續訂。訂閱於每個計費週期自動續訂。使用者可隨時於「帳號 → 訂閱」取消，取消後該週期內仍可使用至到期日，下次將不再扣款，已扣款之續訂費用一般不退費，除非該續訂為錯誤扣款（例如系統錯誤）。',
          '4. 點數加購。一次性購買之點數為數位商品，原則上不退款。如因技術錯誤導致點數未入帳或重複扣款，請於 7 天內聯絡客服協助處理。',
          '5. 服務中斷補償。若 VidGo 因平台故障導致已扣除但未交付之點數，將自動補回相同點數，不另行退費。',
          '6. 退款方式。經核准之退款將原路退回至原付款方式（信用卡 / Apple Pay / Google Pay / 銀行轉帳），通常於 5–10 個工作天內到帳，實際時間依發卡銀行為準。',
          '7. 退款時的點數處理。獲得退款後，與該次付款對應之訂閱點數將自帳戶餘額中扣回；註冊贈送點數與其他購買點數不受影響。若帳戶餘額不足，可能形成負餘額，後續充值將先補負餘額。',
          '8. 法定權利。本政策不影響您依當地法律享有之強制性消費者權利（例如歐盟 14 天冷靜期）。',
          `9. 申請方式。請寄電子郵件至 ${CONTACT_EMAIL}，主旨註明「退款申請」，並附上：帳號電子郵件、訂單編號（可於儀表板查詢）、退款原因。我們將於 5 個工作天內回覆審核結果。`,
        ]
      : [
          `Last updated: ${LAST_UPDATED}`,
          '1. Scope. This refund policy applies to all subscription plans and one-time credit purchases made through the VidGo platform.',
          '2. Subscription Refund Window. New subscribers may request a full refund within 14 calendar days of the first charge, provided they have used no more than 30% of the subscription credits granted for the current cycle. Requests beyond 14 days, or after 30% of cycle credits have been consumed, are considered services rendered and are not eligible.',
          '3. Auto-Renewal. Subscriptions auto-renew each billing cycle. You can cancel anytime under "Account → Subscription"; access continues until the end of the paid period and no further charges are made. Renewal charges are generally non-refundable unless the renewal was clearly erroneous (e.g., system error).',
          '4. One-Time Credit Purchases. One-time credit packs are digital goods and are non-refundable as a general rule. If credits failed to deliver due to a technical error, or you were charged twice, contact support within 7 days for resolution.',
          '5. Service Outage Compensation. If a platform fault deducted credits without delivering output, VidGo will automatically restore the same credit amount; no monetary refund is issued for this case.',
          '6. Refund Method. Approved refunds are returned to the original payment method (credit card / Apple Pay / Google Pay / bank transfer) and typically settle within 5–10 business days, subject to your issuing bank.',
          '7. Credit Reconciliation on Refund. When a refund is granted, the subscription credits tied to that payment are reversed from your balance; sign-up bonus credits and other purchased credits are not affected. If the balance is insufficient, the account may show a negative balance that subsequent purchases will offset first.',
          '8. Statutory Rights. This policy does not affect mandatory consumer rights you have under your local law (for example, the 14-day cooling-off right in the EU).',
          `9. How to Request. Email ${CONTACT_EMAIL} with the subject "Refund Request" and include: account email, order ID (visible in the dashboard), and refund reason. We respond with a decision within 5 business days.`,
        ],
  },
}))
