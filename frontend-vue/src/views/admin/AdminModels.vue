          ...
          <div v-else class="space-y-1">
            ...
            >
              <div class="flex items-center justify-between">
                <div class="font-mono text-sm" style="color: #e8e8f0;">{{ entry.service_key }}</div>
                <span class="text-xs px-2 py-0.5 rounded" :class="sourceBadgeClass(entry.effective.source)">
                  {{ entry.effective.source }}
                </span>
              </div>
              <div class="mt-1 text-xs" style="color: #9494b0;">
                <span class="font-mono">{{ entry.effective.model }}</span>
                <span v-if="entry.effective.version" class="ml-1">v{{ entry.effective.version }}</span>
                <span v-if="isOverridden(entry)" class="ml-2 text-blue-400">• {{ t('adminModels.overridden') }}</span>
              </div>
            </div>
          </div>
        </div>
