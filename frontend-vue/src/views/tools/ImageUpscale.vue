...
async function handleUpscale() {
  if (!uploadedImage.value) {
    uiStore.showWarning('Please upload an image first')
    return
  }

  if (isDemoUser.value) {
    uiStore.showInfo('Subscribe for HD upscale.')
    router.push('/pricing')
    return
  }
  ...
  try {
    ...
    uiStore.showError(err.response?.data?.detail || 'Upscale failed')
  } finally {
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="rounded-xl p-4 flex items-center justify-center min-h-[400px]" style="background: #141420; border: 1px solid rgba(255,255,255,0.06);">
          ...
          <div v-if="!isProcessing && !resultImage" class="text-center" style="color: #6b6b8a;">
            ...
            <p class="text-sm">Upload an image to upscale</p>
          </div>
        </div>
      </div>
    </div>
